...
public class ColorSelector extends PaintedPanel{
	...
	private Timer timer;
	private TimerTask repaintTimerTask, dropTimerTask;
	...
	public ColorSelector(ActionListener confirmAction, Image inImage, ClientPanel cp) {
		...
		timerLabel = new JLabel("Time: 0:" + secondsLeft);
		timerLabel.setFont(FontLibrary.getFont("fonts/kenvector_future_thin.ttf", Font.PLAIN, 28));
	
		repaintTimerTask = new TimerTask(){
			@Override
			public void run() {
				if(!confirmed){
					secondsLeft--;
					if(secondsLeft<10)
						timerLabel.setText("Time: 0:0" + secondsLeft);
					else
						timerLabel.setText("Time: 0:" + secondsLeft);
					repaint();
				}
				else{
		...
		ClearPanel timerPanel = new ClearPanel();
		timerPanel.setBorder(new EmptyBorder(new Insets(50, 10, 0, 30)));
	...
	public void startTimer(){
		timer.scheduleAtFixedRate(repaintTimerTask, 1000, 1000);
		timer.schedule(dropTimerTask, 31000);
	}
