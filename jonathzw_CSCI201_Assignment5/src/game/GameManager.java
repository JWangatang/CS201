...
import library.WavSoundLibrary;
...
public class GameManager {
	...
	public void startAnimation(Tile curr, Tile dest, Pawn p, boolean backWards){
		
		if(timer!=null && livePlayers[currentPlaying].equals(livePlayers[mainPlayer])){
			stopTimer();
		}
		
		Tile currTile = curr;
		
		while(!currTile.equals(dest)){
			currTile.removePawn();
			...
			if(backWards) 
				...
				currTile = currTile.getNext(p.getColor());
		...
		TimerTask animateTask = new TimerTask(){
			...
			public void run(){
				if(currTile.equals(dest)){
					...
					if(!dest.isHome()){
						currTile.setPawn(p);
					} else p.setCurrentTile(dest);
					mGamePanel.repaint();
				}
			}
		};
		
		animationTimer.scheduleAtFixedRate(animateTask, 1000, 250);
		*/
	}
	
	public void stopTimer(){
		
		mGamePanel.timerString = "";
		timer.cancel();
		mGamePanel.repaint();
	}
	...
	public void startTimer(){
		...
		mGamePanel.timerString = "Time 0:15";
		mGamePanel.timerSeconds = 15;
		
		timer = new Timer();
		
		TimerTask repaintTempTimer = new TimerTask(){
			@Override
			public void run() {
				mGamePanel.timerSeconds--;
				if(mGamePanel.timerSeconds<10)
					mGamePanel.timerString = "Time: 0:0" + mGamePanel.timerSeconds;
				else
					mGamePanel.timerString = "Time: 0:" + mGamePanel.timerSeconds;
				mGamePanel.repaint();
			}	
		};
		
		TimerTask passTempTimer = new TimerTask(){
			@Override
			public void run() {
				endTurn();
			}
		};
		timer.scheduleAtFixedRate(repaintTempTimer, 1000, 1000);
		timer.schedule(passTempTimer, 15000);
		mGamePanel.repaint();
	}
	...
	public void drawCard() {
		...
		canDraw = false;
		WavSoundLibrary.playSound("sounds/draw_card.wav");
		CardDialog.popup(card.getType());
		
		if(canMakeMove() && livePlayers[currentPlaying].equals(livePlayers[mainPlayer])){
			startTimer();
		}
	...
	public void endTurn() {
		...
		if(allHome) {
			...
			if(player.equals(Color.RED)){
				if(livePlayers[mainPlayer].getColor().equals(Color.RED)){
					WavSoundLibrary.playSound("sounds/victory.wav");
				}
				else
					WavSoundLibrary.playSound("sounds/lose.wav");
				
				winnerName = "Red";
			}
			else if(player.equals(Color.GREEN)){
				if(livePlayers[mainPlayer].getColor().equals(Color.GREEN)){
					WavSoundLibrary.playSound("sounds/victory.wav");
				}
				else
					WavSoundLibrary.playSound("sounds/lose.wav");
			...
			else if(player.equals(Color.YELLOW)){
				if(livePlayers[mainPlayer].getColor().equals(Color.YELLOW)){
					WavSoundLibrary.playSound("sounds/victory.wav");
				}
				else
					WavSoundLibrary.playSound("sounds/lose.wav");
			...
			else if(player.equals(Color.BLUE)){
				if(livePlayers[mainPlayer].getColor().equals(Color.BLUE)){
					WavSoundLibrary.playSound("sounds/victory.wav");
				}
				else
					WavSoundLibrary.playSound("sounds/lose.wav");
		...
		} else if(card.getType()==CardDeck.TWO)
			WavSoundLibrary.playSound("sounds/your_turn.wav");
	...
	public boolean tileClicked(Tile tile, Player player) {
		...
		if(card.getType() == CardDeck.SEVEN && sevenRemainder != -1) {
			...
			startAnimation(tile, endTile, pawn, false);
			...
			*/
			...
			startAnimation(tile, beforeTile, pawn, true);
			if(!beforeTile.equals(endTile))
				startAnimation(beforeTile, endTile, pawn, false);
			/*
			tile.removePawn();
			if(!endTile.isHome()){
				endTile.setPawn(pawn);
			...
			*/
			...
			if(!livePlayers[currentPlaying].isActive()){
				...
				if(canMoveForward) {
					...
					startAnimation(tile, endTileForward, pawn, false);
					/*
					tile.removePawn();
					...
					*/
					...
					startAnimation(tile, beforeTileBackward, pawn, true);
					
					if(!beforeTileBackward.equals(endTileBackward))
						startAnimation(beforeTileBackward, endTileBackward, pawn, false);
