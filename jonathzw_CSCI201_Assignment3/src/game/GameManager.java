...
public class GameManager {
	...
	public Card getCurrCard(){return card;}
	...
	public void drawCard() {
		...
		JPanel cardPanel = new JPanel(){
			...
			protected void paintComponent(Graphics g){
				...
				g.drawImage(new ImageIcon("card_brown.png").getImage(), 0, 0, getWidth(), getHeight(), null);
			}
		};
		...
		gbc.gridy = 0;
		gbc.ipady = 80;
		JLabel jl = new JLabel(card.getCardNumber());
		jl.setFont(new Font("KenVector Future", Font.BOLD, 24));
		cardPanel.add(jl, gbc);
		
		JTextArea cardText = new JTextArea(card.getDescription());
		cardText.setFont(new Font("KenVector Future", Font.BOLD, 11));
		...
		gbc.gridx = 0;
		gbc.gridy = 1;
		cardPanel.add(cardText, gbc);
		
		cardDialog.add(cardPanel);
		cardDialog.setSize(200, 300);
		cardDialog.setLocation(200, 300);
	...
	private void simulatePlayer(Player player) {
		...
		//If the bot can move a piece out of start, it will do so
		...
		if(card.getType() == CardDeck.SEVEN && sevenRemainder != -1) {
			...
			if(currentPlaying != mainPlayer) {
				...
				if(canMoveForward) {
					...
					if(endTileBackward.isOccupied())
						players[GameHelpers.getIndexFromColor(endTileBackward.getPawnColor())].returnPawn(endTileBackward.getPawn());
					tile.removePawn();
	...
	public int calculatePlayerScore(){
		int score = 0;
		for(Pawn p: getMainPlayer().getPawns()){
			if(p.getCurrentTile().isHome())
				score+=5;	
		}
		
		for(Player p: livePlayers){
			if(!p.equals(getMainPlayer())){
				for(Pawn pawn : p.getPawns()){
					if(!pawn.getCurrentTile().isHome())
						score+=3;
					if(pawn.getCurrentTile().isStart())
						score+=1;
				}
